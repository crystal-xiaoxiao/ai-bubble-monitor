...
```
red_pct = red_count / 24 * 100
weighted_risk_score = (red_count + yellow_count * 0.5) / 24 * 100

red_pct >= 60  → 系统性顶部信号（建议进入防御）  · 红灯
red_pct >= 40  → 高风险预警（结构性风险显著）   · 橙灯
red_pct >= 25  → 中度警戒（风险积累中）         · 黄灯
否则           → 观察期（风险可控）             · 绿灯
```

### 分类强制升级规则

历史上估值 + 资金面同时大面积亮红是泡沫顶部最一致的领先信号（早于广度恶化、信用事件、会计造假）。所以即使整体 red_pct 没到 40%，只要满足下面任一条件，**判读必须强制升级到至少"高风险预警"**：

```
触发条件 A: 估值类 ≥2/3 红 (CAPE / Top-5 / NVDA FPE 中至少 2 个红)
       AND 资金面类 ≥3/4 红 (Capex YoY / FCF YoY / VC AI 占比 / NVDA 客户投资 中至少 3 个红)
       → 升级到 "高风险预警"

如果当前已 ≥40%（基础判读本身就是"高风险预警"或"系统性顶部"），则取较高的那个。
```

升级时 verdict_desc 末尾要加一句：「估值 + 资金面双类共振触发分类强制升级」，让用户知道是分类规则而非整体比例触发的。
