...
**2026-08-28 本副本已修改、线上 routine 尚未同步**，改动两处（都在下方「线上 prompt」正文里）：
1. 「4. 聚合」verdict_desc 段新增**排版格式条款**（`\n\n` 分 4-6 段、TL;DR 首段前置、段首 `**小标题**`、只允许 `**` 记号、目标长度中文 800-1500 字/英文 500-900 词）
2. 「7. 写飞书 payload」verdict_desc 行新增：写 outbox 前删掉 `**` 记号（飞书原样显示星号）

请在 **8/31（周日）23:00 UTC 本期 routine 运行前**同步到线上（https://claude.ai/code/routines 或 Claude Code `/schedule`）。未同步的影响：网页端不受影响（前端已兼容新旧格式），仅飞书推送继续显示 `**` 星号、总结长度不收敛。
